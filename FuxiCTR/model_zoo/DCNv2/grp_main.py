...
def main():
    ...
    output_dir = './yaml_moc_1019_experiments_test'
    os.makedirs(output_dir, exist_ok=True)

    expid = 'base'
    base_config = load_config(base_config_dir, expid)

    for seed in [20,201,1027,2024,2333][:1]:
        for lr in [0.001,0.005,0.01][:1]:
            for dataset in ['beauty','sports','toys'][:1]:
                for method in ['base','me', 'moc', 'rq'][1:2]:
                    for scala in [0,1,3,7][1:2]:
                        ...
                        config.update({ #'cov_weight': cov_weight,
                                        ...
                                        'seed': seed,
                                        'model_root': 'moc_1019_experiments_test',
                                        'dataset_id': dataset,
