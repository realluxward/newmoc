...
class BaseModel(nn.Module):
    ...
    def train_epoch(self, data_generator):
        ...
        if self._verbose == 0:
            ...
            batch_iterator = tqdm(data_generator, ncols=50)
    ...
    def evaluate(self, data_generator, metrics=None):
        ...
        with torch.no_grad():
            ...
            group_id = []
            if self._verbose > 0:
                data_generator = tqdm(data_generator, ncols=50)
            ...
            logging.info('[Metrics] ' + ' - '.join('{}: {:.4f}'.format(k, v) for k, v in val_logs.items()))
            return val_logs
    ...
    def dump_feat(self, data_generator):
        extend = False
        if extend:
            dump_keys = ['moe_cid_1']
        else:
            dump_keys = ['item_id', 'user_id','brand', 'categories','sales_type'] + [f'moe_cid_{i}' for i in range(4)]
        # dump_keys = ['item_id', 'user_id']
        from collections import defaultdict
        results = defaultdict(list)
        for batch_data in data_generator:
            results_dict = self.get_emb(batch_data, dump_keys)
            for k in results_dict.keys():
                results[k].append(results_dict[k])
        print("Stacking...")
        for k in results_dict.keys():
        ...
        for k in results_dict.keys():
            if extend:
                torch.save(results[k], f"./feat/emb/sample_extend_{k}_{results[k].shape[1]}_64.pt")
            else:
                torch.save(results[k], f"./feat/emb/sample_{self.prefix}_{k}_64.pt")
    
    def get_ids(self, inputs, key):
        y = inputs[:, self.feature_map.get_column_index(key)].to(self.device)
        return y.view(-1, 1)

    def dump_id(self,data_generator):
        # dump_keys = ['item_id', 'user_id','brand', 'categories','sales_type','moe_cid_1', 'label']
        dump_keys = ['label']
        from collections import defaultdict
        results = defaultdict(list)
        for batch_data in data_generator:
            for k in dump_keys:
                results[k].append(self.get_ids(batch_data, k))
        print("Stacking...")
        for k in dump_keys:
            results[k] = torch.cat(results[k], dim=0)
        print("Saving...")
        for k in dump_keys:
            torch.save(results[k], f"./feat/id/sample_{self.prefix}_{k}_64.pt")

    def dump_layer_emb(self, data_generator):
        from collections import defaultdict
        results = defaultdict(list)
        for batch_data in data_generator:
            batch_result = self.get_layer_emb(batch_data)
            for k in batch_result.keys():
                results[k].append(batch_result[k])
        print("Stacking...")
        for k in batch_result.keys():
            results[k] = torch.cat(results[k], dim=0)
        print("Saving...")
